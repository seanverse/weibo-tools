 ...
 * @author Daniel Qian
 */
public class WeiboMessageInMemoryDuplicateChecker implements WeiboMessageDuplicateChecker {
   ...
   * </pre>
   */
  public WeiboMessageInMemoryDuplicateChecker() {
   ...
   */
  public WeiboMessageInMemoryDuplicateChecker(Long timeToLive, Long clearPeriod) {
  ...
  protected void checkBackgroundProcessStarted() {
    ...
    Thread t = new Thread(new Runnable() {
      ...
      public void run() {
        try {
          while (true) {
            Thread.sleep(WeiboMessageInMemoryDuplicateChecker.this.clearPeriod);
            Long now = System.currentTimeMillis();
            for (Map.Entry<String, Long> entry :
              WeiboMessageInMemoryDuplicateChecker.this.msgId2Timestamp.entrySet()) {
              if (now - entry.getValue() > WeiboMessageInMemoryDuplicateChecker.this.timeToLive) {
                WeiboMessageInMemoryDuplicateChecker.this.msgId2Timestamp.entrySet().remove(entry);
              }
            }
          }
