...
import vip.seanxq.weibo.common.util.http.SimpleGetRequestExecutor;
...
public class ApacheSimpleGetRequestExecutor extends SimpleGetRequestExecutor<CloseableHttpClient, HttpHost> {
  ...
  @Override
  public String execute(String uri, String queryParam, WeiboType weiboType) throws WeiboErrorException, IOException {
    if (queryParam != null) {
      if (uri.indexOf('?') == -1) {
        uri += '?';
      }
    ...
    try (CloseableHttpResponse response = requestHttp.getRequestHttpClient().execute(httpGet)) {
      ...
      return handleResponse(weiboType, responseContent);
    } finally {
