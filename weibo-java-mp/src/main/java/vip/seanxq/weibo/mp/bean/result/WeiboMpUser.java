 ...
 */
@Data
public class WeiboMpUser implements Serializable {
   ...
   * <pre>
   * 只有在将公众号绑定到微博开放平台帐号后，才会出现该字段。
   * 另外，在用户未关注公众号时，将不返回用户unionID信息。
   * 已关注的用户，开发者可使用“获取用户基本信息接口”获取unionID；
   * 未关注用户，开发者可使用“微博授权登录接口”并将scope参数设置为snsapi_userinfo，获取用户unionID
   * </pre>
   */
  ...
  /**
   * 用户特权信息，json 数组，如微博沃卡用户为（chinaunicom）.
   */
  ...
  public static WeiboMpUser fromJson(String json) {
    return WbMpGsonBuilder.create().fromJson(json, WeiboMpUser.class);
  }

  public static List<WeiboMpUser> fromJsonList(String json) {
    Type collectionType = new TypeToken<List<WeiboMpUser>>() {
    }.getType();
