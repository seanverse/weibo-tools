...
@Slf4j
@XStreamAlias("xml")
public class WeiboMpXmlMessage implements Serializable {
  ...
  ///////////////////////
  // 以下都是微博推送过来的消息的xml的element所对应的属性
  ///////////////////////
   ...
   * 另外：
   * 官网文档中，微博卡券>>卡券事件推送>>2.7 进入会员卡事件推送 user_view_card
  ...
  /**
   * 核销来源.
   * 支持开发者统计API核销（FROM_API）、公众平台核销（FROM_MP）、卡券商户助手核销（FROM_MOBILE_HELPER）（核销员微博号）
   */
  ...
  /**
   * <pre>
   * 官网文档中，微博卡券>>卡券事件推送>>2.10 库存报警事件card_sku_remind
   * Detail：报警详细信息.
   * </pre>
   */
  @XStreamAlias("Detail")
  ...
  /**
   * <pre>
   * 官网文档中，微博卡券>>卡券事件推送>>2.9 会员卡内容更新事件 update_member_card
   * ModifyBonus：变动的积分值.
   * </pre>
   */
  ...
  /**
   * <pre>
   * 官网文档中，微博卡券>>卡券事件推送>>2.6 买单事件推送 User_pay_from_pay_cell
   * TransId：微博支付交易订单号（只有使用买单功能核销的卡券才会出现）.
   * </pre>
   */
   ...
   * 认证过期失效通知: 有效期 (整形)，指的是时间戳，表示已于该时间戳认证过期，需要重新发起微博认证
   */
  ...
  /**
   * 微博客户端生成的session id，用于request和response对应，
  ...
  public static WeiboMpXmlMessage fromXml(String xml) {
    //修改微博变态的消息内容格式，方便解析
    xml = xml.replace("</PicList><PicList>", "");
    final WeiboMpXmlMessage xmlMessage = XStreamTransformer.fromXml(WeiboMpXmlMessage.class, xml);
    xmlMessage.setAllFieldsMap(XmlUtils.xml2Map(xml));
    return xmlMessage;
  }

  public static WeiboMpXmlMessage fromXml(InputStream is) {
    return XStreamTransformer.fromXml(WeiboMpXmlMessage.class, is);
  }
   ...
   * @param weiboConfigStorage 配置存储器对象
   * @param timestamp         时间戳
   * @param nonce             随机串
   * @param msgSignature      签名串
   */
  public static WeiboMpXmlMessage fromEncryptedXml(String encryptedXml, WeiboConfigStorage weiboConfigStorage,
                                                   String timestamp, String nonce, String msgSignature) {
    WeiboMpCryptUtil cryptUtil = new WeiboMpCryptUtil(weiboConfigStorage);
    String plainText = cryptUtil.decrypt(msgSignature, timestamp, nonce, encryptedXml);
    log.debug("解密后的原始xml消息内容：{}", plainText);
    return fromXml(plainText);
  }

  public static WeiboMpXmlMessage fromEncryptedXml(InputStream is, WeiboConfigStorage weiboConfigStorage, String timestamp,
                                                   String nonce, String msgSignature) {
    try {
      return fromEncryptedXml(IOUtils.toString(is, StandardCharsets.UTF_8), weiboConfigStorage, timestamp, nonce, msgSignature);
    } catch (IOException e) {
   ...
   * <pre>
   * 当接受用户消息时，可能会获得以下值：
   * {@link WeiboConsts.XmlMsgType#TEXT}
   * {@link WeiboConsts.XmlMsgType#IMAGE}
   * {@link WeiboConsts.XmlMsgType#VOICE}
   * {@link WeiboConsts.XmlMsgType#VIDEO}
   * {@link WeiboConsts.XmlMsgType#LOCATION}
   * {@link WeiboConsts.XmlMsgType#LINK}
   * {@link WeiboConsts.XmlMsgType#EVENT}
   * </pre>
   */
   ...
   * {@link WeiboConsts.XmlMsgType#VOICE}
   * {@link WeiboConsts.XmlMsgType#VIDEO}
   * {@link WeiboConsts.XmlMsgType#NEWS}
   * {@link WeiboConsts.XmlMsgType#MUSIC}
   * </pre>
   */
