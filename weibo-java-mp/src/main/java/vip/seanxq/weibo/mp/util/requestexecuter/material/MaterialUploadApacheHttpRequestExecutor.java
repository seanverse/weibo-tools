...
public class MaterialUploadApacheHttpRequestExecutor extends MaterialUploadRequestExecutor<CloseableHttpClient, HttpHost> {
  ...
  @Override
  public WeiboMpMaterialUploadResult execute(String uri, WeiboMpMaterial material, WeiboType weiboType) throws WeiboErrorException, IOException {
    ...
    if (material == null) {
      throw new WeiboErrorException(WeiboError.builder().errorCode(-1).errorMsg("非法请求，material参数为空").build());
    }
    ...
    if (material.getForm() != null) {
      multipartEntityBuilder.addPart("description",
        new StringBody(WeiboGsonBuilder.create().toJson(form), ContentType.create("text/plain", Consts.UTF_8)));
    }
