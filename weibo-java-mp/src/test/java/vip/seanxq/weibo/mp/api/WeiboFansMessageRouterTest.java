 ...
 */
@Test
public class WeiboFansMessageRouterTest {

  @Test(enabled = false)
  public void prepare(boolean async, StringBuffer sb, WeiboFansMessageRouter router) {
    router
      .rule()
      .async(async)
      .msgType(WeiboConsts.XmlMsgType.TEXT).event(WeiboConsts.EventType.CLICK).eventKey("KEY_1").content("CONTENT_1")
      .handler(new WeiboFansMessageHandler(sb, "COMBINE_4"))
      .end()
      .rule()
      .async(async)
      .msgType(WeiboConsts.XmlMsgType.TEXT).event(WeiboConsts.EventType.CLICK).eventKey("KEY_1")
      .handler(new WeiboFansMessageHandler(sb, "COMBINE_3"))
      .end()
      .rule()
      .async(async)
      .msgType(WeiboConsts.XmlMsgType.TEXT).event(WeiboConsts.EventType.CLICK)
      .handler(new WeiboFansMessageHandler(sb, "COMBINE_2"))
      .end()
      .rule().async(async).msgType(WeiboConsts.XmlMsgType.TEXT).handler(new WeiboFansMessageHandler(sb, WeiboConsts.XmlMsgType.TEXT)).end()
      .rule().async(async).event(WeiboConsts.EventType.CLICK).handler(new WeiboFansMessageHandler(sb, WeiboConsts.EventType.CLICK)).end()
      .rule().async(async).eventKey("KEY_1").handler(new WeiboFansMessageHandler(sb, "KEY_1")).end()
      .rule().async(async).eventKeyRegex("KEY_1*").handler(new WeiboFansMessageHandler(sb, "KEY_123")).end()
      .rule().async(async).content("CONTENT_1").handler(new WeiboFansMessageHandler(sb, "CONTENT_1")).end()
      .rule().async(async).rContent(".*bc.*").handler(new WeiboFansMessageHandler(sb, "abcd")).end()
      .rule().async(async).matcher(new WeiboFansMessageMatcher() {
      @Override
      public boolean match(WeiboMpXmlMessage message) {
        return "strangeformat".equals(message.getFormat());
      }
    }).handler(new WeiboFansMessageHandler(sb, "matcher")).end()
      .rule().async(async).handler(new WeiboFansMessageHandler(sb, "ALL")).end();
  }

  @Test(dataProvider = "messages-1")
  public void testSync(WeiboMpXmlMessage message, String expected) {
    StringBuffer sb = new StringBuffer();
    WeiboFansMessageRouter router = new WeiboFansMessageRouter(null);
  ...
  public void testExternalExcutorService(WeiboMpXmlMessage message, String expected) throws InterruptedException {
    ...
    WeiboFansMessageRouter router = new WeiboFansMessageRouter(null, executorService);
    prepare(true, sb, router);
    router.route(message);
    Thread.sleep(500);
  ...
  public void testConcurrency() throws InterruptedException {
    final WeiboFansMessageRouter router = new WeiboFansMessageRouter(null);
    router.rule().handler(new vip.seanxq.weibo.mp.api.WeiboFansMessageHandler() {
      @Override
  ...
  public static class WeiboFansMessageHandler implements vip.seanxq.weibo.mp.api.WeiboFansMessageHandler {

    private StringBuffer sb;
    private String echoStr;

    public WeiboFansMessageHandler(StringBuffer sb, String echoStr) {
      this.sb = sb;
  ...
  public static class WeibSessionMessageHandler implements vip.seanxq.weibo.mp.api.WeiboFansMessageHandler {
